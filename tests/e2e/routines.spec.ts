...
test('crea una scheda e riordina la copia senza cambiare l’originale, anche dopo la riapertura', async ({ page }) => {
  ...
  await page.getByLabel('Ripetizioni min.', { exact: true }).fill('6');
  await page.getByLabel('Ripetizioni max.', { exact: true }).fill('8');
  await page.getByLabel('Recupero, secondi', { exact: true }).fill('120');
  ...
  await expect(page.getByLabel('Ripetizioni min.', { exact: true })).toHaveValue('6');
  await expect(page.getByLabel('Ripetizioni max.', { exact: true })).toHaveValue('8');
  await expect(page.getByLabel('Recupero, secondi', { exact: true })).toHaveValue('120');
...
test('modificare scheda e catalogo conserva nomi, convenzioni e serie nello storico già completato', async ({ page }) => {
  ...
  await page.getByLabel('Serie', { exact: true }).fill('5');
  await page.getByLabel('Ripetizioni min.', { exact: true }).fill('6');
  await page.getByLabel('Ripetizioni max.', { exact: true }).fill('7');
