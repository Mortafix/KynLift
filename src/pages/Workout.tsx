...
import { formatNumber, initialSet, previousSet, sessionCompletedSets, validateSet } from '../lib/domain';
...
function initialFields(entry: SetEntry): SetFields {
  return { weight: String(entry.weight).replace('.', ','), reps: String(entry.reps), rir: entry.rir === null ? '' : String(entry.rir), rightWeight: entry.rightWeight === null ? '' : String(entry.rightWeight).replace('.', ','), rightReps: entry.rightReps === null ? '' : String(entry.rightReps), separateSides: entry.rightWeight !== null || entry.rightReps !== null };
}
...
function NumberField({ label, unit, value, onChange, step, id, max = 2000, min = 0, large = true, disabled = false, active = false, onActivate }: { label: string; unit: string; value: string; onChange: (value: string) => void; step: number; id: string; max?: number; min?: number; large?: boolean; disabled?: boolean; active?: boolean; onActivate?: () => void }) {
  ...
  return <div className={`number-field ${large ? '' : 'number-field-small'}`} data-active={active} onFocusCapture={onActivate}>
    <div className="number-field-heading"><label htmlFor={id}>{label}</label><span className="number-unit">{unit}</span></div>
    <div className="number-controls"><button type="button" aria-label={`Diminuisci ${label.toLowerCase()}`} disabled={disabled || Number.isFinite(numeric) && numeric <= min} onClick={() => adjust(-1)}><Minus size={22} /></button><input id={id} aria-label={label} aria-required="true" value={value} placeholder="—" data-long={value.length > 4} disabled={disabled} onChange={(event) => onChange(event.target.value)} inputMode={id.includes('reps') || id.includes('rir') ? 'numeric' : 'decimal'} autoComplete="off" onFocus={(event) => event.target.select()} maxLength={7} /><button type="button" aria-label={`Aumenta ${label.toLowerCase()}`} disabled={disabled || numeric >= max} onClick={() => adjust(1)}><Plus size={22} /></button></div>
  </div>;
}
...
export function Workout({ session, onBack, onFinish, onDirtyChange }: WorkoutProps) {
    ...
    await run(async () => {
      await write((current) => {
        ...
        return [
          ...
          { collection: 'sessions', value: { ...current, currentSet: current.currentExercise === exerciseIndex ? Math.min(current.currentSet, count - 2) : current.currentSet, exercises: current.exercises.map((item) => item.id === exercise.id ? { ...item, target: { ...item.target, sets: count - 1 } } : item), updatedAt: now } },
        ];
      });
  ...
  return <section className="page workout-page">
    ...
    <div className="workout-layout"><div className="workout-primary">
      ...
      <div className="workout-facts"><p className="workout-target"><span>Obiettivo</span><strong>{exercise.target.repsMin === exercise.target.repsMax ? exercise.target.repsMin : `${exercise.target.repsMin}–${exercise.target.repsMax}`} ripetizioni{exercise.snapshot.unilateral ? ' / lato' : ''}</strong></p>
